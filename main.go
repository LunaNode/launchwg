...
// Ubuntu 22.04 64-bit (template)
const IMAGE_ID = 630900
...
func main() {
	scriptBytes, err := ioutil.ReadFile("cloud-config")
	if err != nil {
		panic(err)
	}
	...
	http.HandleFunc("/launch", func(w http.ResponseWriter, r *http.Request) {
		...
		if strings.HasSuffix(hostname, ".lndyn.com") && strings.HasPrefix(hostname, "wg") && len(hostname) == 18 {
